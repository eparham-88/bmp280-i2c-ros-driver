...
#include <ros/ros.h>
#include <sensor_msgs/PressureTemperatureAltitude.h>

#define sleepms(ms)  usleep((ms)*1000)
#define I2CBus             "/dev/i2c-0"


double pressure_at_sea_level_ = 101.325; // in [kPa]
...
typedef enum {
	OSRS_TEMPERATURE_SKIPPED = 0,  // temperature measurement skipped
	OSRS_TEMPERATURE_16BITS = 1,   // 16 Bits, x1
	OSRS_TEMPERATURE_17BITS = 2,   // 17 Bits, x2
	OSRS_TEMPERATURE_18BITS = 3,   // 18 Bits, x4
	OSRS_TEMPERATURE_19BITS = 4,   // 19 Bits, x8
	OSRS_TEMPERATURE_20BITS = 5,   // 20 Bits, x16
} tOsrsTemperature;

// Pressure resolution
typedef enum {
	OSRS_PRESSURE_SKIPPED = 0,  // pressure measurement skipped
	OSRS_PRESSURE_16BITS = 1,   // 16 Bits, x1, ultra low power
	OSRS_PRESSURE_17BITS = 2,   // 17 Bits, x2, low power
	OSRS_PRESSURE_18BITS = 3,   // 18 Bits, x4, standard resolution
	OSRS_PRESSURE_19BITS = 4,   // 19 Bits, x8, high resolution
	OSRS_PRESSURE_20BITS = 5,   // 20 Bits, x16, ultra high resolution
} tOsrsPressure;

// filter settings
typedef enum {
	FILTER0 = 0, // Filter Off
	FILTER1 = 1, // 2
	FILTER2 = 2, // 4
	FILTER3 = 3, // 8
	FILTER4 = 4, // 16
...
double Altitude( double static_pressure )
{
  // fancy math: barometric equation
  return 44330 * (1 - pow( ((static_pressure/10.0) / pressure_at_sea_level_), (1/5.255) ));
}
...
int main(int argc, char* argv[]){
	...
	if (use_case == 0) {
		...
		bmp280rc = bmp280_init(fd, softReset, POWER_MODE_NORMAL, STANDBY_0000_5_MS, FILTER4, OSRS_TEMPERATURE_16BITS, OSRS_PRESSURE_18BITS);
		sampling_rate = 83.3;
	} else if (use_case == 1) {
		/* Drop Detection */
		bmp280rc = bmp280_init(fd, softReset, POWER_MODE_NORMAL, STANDBY_0000_5_MS, FILTER0, OSRS_TEMPERATURE_16BITS, OSRS_PRESSURE_17BITS);
		sampling_rate = 125;
	} else {
		bmp280rc = bmp280_init(fd, softReset, POWER_MODE_NORMAL, STANDBY_0000_5_MS, FILTER4, OSRS_TEMPERATURE_17BITS, OSRS_PRESSURE_20BITS);
		sampling_rate = 26.3;
	}
    ...
    pub_data = nh->advertise<sensor_msgs::PressureTemperatureAltitude>("bar/data", 1);
	int seq = 0;
    std::unique_ptr<ros::Rate> rate =std::make_unique<ros::Rate>((double)sampling_rate);
		...
		if(bmp280_ReadReg(fd, BMP280_DEVICE_ADDRESS, BMP280_REGISTER_DIG_T1, data, 24) == 0) {
			...
			while(ros::ok()) {
				...
				if(bmp280_ReadReg(fd, BMP280_DEVICE_ADDRESS, BMP280_REGISTER_PRESSDATA_MSB, data, 6) == 0) {
					...
					sensor_msgs::PressureTemperatureAltitude msg;
					msg.header.stamp = time;
					msg.header.frame_id = "BMP280_I2C";
					msg.header.seq = seq;
					msg.Pressure = (double)pressureDouble/25600;
					msg.Temperature = (double)temperature/100.000d;
					msg.Altitude = Altitude(msg.Pressure);
					...
					// printf("{\"temperature\":%2.8lf,\"PressureDouble\":%4.8f}\n",
					// 	temperature/100.000d, pressureDouble/25600);
